...
export const createImage = async (
    ...
    try {
        ...
        if (typeof ids === 'string') {
            ...
            images.map((img, i) => imagesWithId.unshift({ images: img, id: ids[i] }));
        }
        for (const image of imagesWithId) {
            ...
            if (findModelId != null) {
                ...
                resultImage.unshift({ result: `The ${image.id} has already registered` });
            }
            const saveInCloudinary = await cloudinary.uploadFile(image.images.path, folderName);
