...
export const updateProject = async (
   projectId: string,
   project: interfaces.IUpateProject
) => {
   try {
      const findProject = await model.findById(projectId).lean();
      if (findProject === null)
         return `The ${projectId} id not found in the database`;
...
export const getProjectByToolId = async (
   toolId: string,
   page: number = 1,
   limit: number = 10
) => {
   try {
      ...
      if (typeof findPI === 'object') {
         ...
         for (const i of findPI.result) {
            ...
            const findApp = await appService.getOneApplication(
               findProject[0]._id
            );
         ...
         const findTool = await toolService.getOneTool(
            findPI.result[0].toolsId
         );
         newData = {
            tool: findTool,
            result,
